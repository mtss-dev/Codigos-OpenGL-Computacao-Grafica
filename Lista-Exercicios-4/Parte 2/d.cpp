...
float h(int i, int j)
{
   float radius = 0.5; // Radius of the wave.
   float amplitude = 0.2; // Amplitude of the wave.
   float frequency = 5.0; // Frequency of the wave.
   return ( -1 + 2*(float)j/q + amplitude * sin(frequency * ((float)i/p) * 2 * PI) * exp(-((float)j/q - 0.5)*((float)j/q - 0.5) / (2 * radius * radius)) );
}
